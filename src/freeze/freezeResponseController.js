    ...
    freezeResponse: async function (json) {
        try {
            ...
            for (const element of json) {
                if (element.type.localeCompare(constant.freezeBucket.PAYROLL_OUTPUT) == 0) {
                    await readDataFromS3(element, constant.freezeBucket.PAYROLL_OUTPUT);
                } else if (element.type.localeCompare(constant.freezeBucket.ACCOUNT_OUTPUT) == 0) {
                    await readDataFromS3(element, constant.freezeBucket.ACCOUNT_OUTPUT);
                }
            }
...
async function updateFreezeResponse(data, freezeState, type) {
    try {
        // Split rows by new line
        let csvData = data.split("\n");
        for (let i = 0; i < csvData.length; i++) {
            let element = csvData[i];
            let rowNumber = parseInt(element.split(" ")[0]);
            if (!neritoUtils.isEmpty(sentFileMap.get(rowNumber))) {
                ...
                if (type.localeCompare(constant.freezeBucket.PAYROLL_OUTPUT) == 0) {
                    const result = await payrollService.updatePayroll(Id, SK, freezeState);
                } else if (type.localeCompare(constant.freezeBucket.ACCOUNT_OUTPUT) == 0) {
                    const result = await employeeService.updateEmplyee(Id, SK, freezeState);
                }
            }
        }
    ...
    try {
        ...
        for (const element of json) {
            if (neritoUtils.isEmpty(element.type)) {
                return neritoUtils.errorResponseJson("Type can not be empty at position " + count, 400);
            } else if (element.type != constant.freezeBucket.ACCOUNT_OUTPUT && element.type != constant.freezeBucket.PAYROLL_OUTPUT) {
                return neritoUtils.errorResponseJson("Invalid type at position " + count, 400);
            }
            if (neritoUtils.isEmpty(element.orgId)) {
                return neritoUtils.errorResponseJson("orgId can not be empty at position " + count, 400);
            } else if (!(organizationsList.includes(element.orgId))) {
                return neritoUtils.errorResponseJson("Incorrect orgId at position " + count, 400);
            }
            if (neritoUtils.isEmpty(element.filename)) {
                return neritoUtils.errorResponseJson("filename can not be empty at position " + count, 400);
            }
            count++;
        }
    } catch (error) {
...
async function readDataFromS3(json, type) {
    ...
    try {
        ...
        const tempFreezeBucket = freeze_temp_bucket + "/" + type + "/IN/" + json.orgId;

        // Get txt data from temp folder (as we need to remember row number which we have sent to bank)
        const sentCsvFile = await service.readFreezeResponseFilesFromS3(sentFilename, tempFreezeBucket, payrollNeritoBucketRegion);
        ...
        const freezeBucket = payrollDisbursementFilesBucket + "/" + type + "/OUT/" + json.orgId;

        // Get txt data from OUT folder
        const responseCsvFile = await service.readFreezeResponseFilesFromS3(json.filename, freezeBucket, payrollDisbursementFilesBucketRegion);
        await updateFreezeResponse(responseCsvFile.Body.toString(), constant.freezeState.SUCCESS, constant.freezeBucket.PAYROLL_OUTPUT);
        if (!neritoUtils.isEmpty(json.errorFilename)) {
            responseCsvFile = await service.readFreezeResponseFilesFromS3(json.errorFilename, freezeBucket, payrollDisbursementFilesBucketRegion);
            await updateFreezeResponse(responseCsvFile.Body.toString(), constant.freezeState.Error, constant.freezeBucket.PAYROLL_OUTPUT);
        }
    } catch (error) {
        console.error("Something went wrong while fetching data from temp folder", error);
        throw "Something went wrong";
    }
}
