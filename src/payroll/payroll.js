...
async function insertPayroll(orgId, fileId) {
    try {
        ...
        try {
            ...
            if (fileDetails.CsvStatus.localeCompare(constant.csvStatus.PENDING) == 0) {
        ...
        try {
            ...
            console.error("Failed to delete old payroll data of this month by orgId : " +orgId );
            throw "Something went wrong";
        }
        ...
        try {
            ...
            const result = await payrollService.insertDataIntoDb(csvJson, organization);
            if (!neritoUtils.isEmpty(result)) {
